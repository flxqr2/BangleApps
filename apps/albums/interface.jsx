...
const diffusions = {
  ...
  errorrandom:
    'Randomised Error Diffusion',
  bayer2: '2x2 Bayer',
  bayer4: '4x4 Bayer',
};

const autoCropImg = async (
  img,
  settings
) => {
  let l = 0,
  ...
  const firstColor = ctx.getImageData(
    0,
    0,
    1,
    1
  ...
  const equalColors = (
    ...
    Math.abs(a[0 + aOffset] - b[0]) <=
      threshold &&
    Math.abs(a[1 + aOffset] - b[1]) <=
      threshold &&
    Math.abs(a[2 + aOffset] - b[2]) <=
      threshold &&
    Math.abs(a[3 + aOffset] - b[3]) <=
      threshold;
  ...
  const { x, y } = {
    x:
      (dim - (canvas.width - l - r)) *
        0.5 -
      l,
    y:
      (dim - (canvas.height - t - b)) *
        0.5 -
      t,
  };
...
const checksum = (s) => {
  ...
  return (chk & 0xffffffff).toString(
    16
  );
};
...
const uid = () => {
  ...
  return (
    ff() + ff(true) + ff(true) + ff()
  );
};

const ViewOnBangleButton = memo(
  ({ data, ...props }) => {
    ...
    >
      <label
        ...
        style={{
          ...
          textAlign: 'center',
          boxShadow:
            'black 0px -3px 4px -4px',
          cursor: 'pointer',
        }}
      >
        Drop image files here or{' '}
        <u>click</u> to add images
        <UploadField
...
const BangleConnect = ({
  ...
  const {
    ...
    const {
      ...
      jsonData,
      deleteFiles,
    } = convertToBangleJson(albums);
    const json =
      JSON.stringify(jsonData);

    checksums.current.current =
      checksum(json);
    ...
    useCallback(() => {
      ...
      UART.eval(
        ...
        (albums, err) => {
          ...
          checksums.current.original =
            checksum(
              JSON.stringify(_albums)
            );
...
const AlbumImage = ({
  ...
  const [loading, setLoading] =
    useState(true);
  const [bangleData, setBangleData] =
    useState(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const url =
        imageconverter.stringToImageURL(
          str
        );
      setUrl(url);
    } catch (err) {
